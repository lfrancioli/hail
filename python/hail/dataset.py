...
class VariantDataset(object):
    ...
    def delete_va_attribute(self, ann_path, attribute):
        ...
        Attributes are key/value pairs that can be attached to a variant annotation field. See :py:meth:`~hail.VariantDataset.`set_va_attribute` for details.


        :param str ann_path: Variant annotation path with `va`.

        :param str attribute: The attribute to remove (key).

        :return: Variant dataset with the attribute removed.
        :rtype: :class:`.VariantDataset`
        """
    ...
    def get_va_attributes(self, ann_path):
        """Returns a dict of all attributes attached to a variant annotation.

        Attributes are key/value pairs that can be attached to a variant annotation field. See :py:meth:`~hail.VariantDataset.`set_va_attribute` for details.

        :param str ann_path: Variant annotation path beginning with `va`.

        :return: Dict of all attributes attached to variant annotation.
        :rtype: dict
        """
    ...
    def set_va_attribute(self, ann_path, key, value):
        ...
        * INFO fields attributes (attached to (`va.info.*`)):
            ...
            * 'Number': The arity of the field. When importing, the value is read from the VCF INFO field definition. When exporting, the default value is ``0`` for *Boolean*, ``.`` for *Array* and ``1`` for all other types.
        ...
        * FILTER entries in the VCF header are generated based on the attributes of ``va.filters``. Each key/value attribute will generate a FILTER entry in the VCF with ``ID = key`` and ``Description = value``.

        **Examples**

        Consider the following command which adds a filter and an annotation to a split VDS:
        ...
        2. a FILTER field ``HardFilter`` that filters all sites with the [GATK-recommended hard filters](http://gatkforums.broadinstitute.org/gatk/discussion/2806/howto-apply-hard-filters-to-a-call-set):

            * For SNPs: ``QD < 2.0 || FS < 60 || MQ < 40 || MQRankSum < -12.5 || ReadPosRankSum < -8.0``
            * For Indels: ``QD < 2.0 || FS < 200.0 || ReadPosRankSum < 20.0``

        >>> vds = (hc.import_vcf('data/sample.vcf')
        ...     .annotate_variants_expr(['va.info.AC_HC = gs.filter(g => g.dp >= 10 && g.gq >= 20).callStats(g => v).AC[1:]',
        ...         'va.filters = if ((v.altAllele.isSNP && (va.info.QD < 2.0 || va.info.FS < 60 || va.info.MQ < 40 || va.info.MQRankSum < -12.5 || va.info.ReadPosRankSum < -8.0)) || (va.info.QD < 2.0 || va.info.FS < 200.0 || va.info.ReadPosRankSum < 20.0)) va.filters.add("HardFilter") else va.filters']))

        Exporting this VDS as VCF would produce the following header for these new fields:
        ...
        * There is no FILTER entry for ``HardFilter``.

        * Since ``AC_HC`` has one entry per non-reference allele, its ``Number`` should be ``A``.

        * ``AC_HC`` should have a Description.

        We can fix this by setting the attributes of these fields:

        >>> vds = vds.set_va_attributes('va.info.AC_HC', 'Description', 'Allele count for high quality genotypes (DP >= 10, GQ >= 20)')
        >>> vds = vds.set_va_attributes('va.info.AC_HC', 'Number', 'A')
        >>> vds = vds.set_va_attributes('va.filters', 'HardFilter', 'This site fails GATK suggested hard filters.')
        ...
        :return: Variant dataset with attributes added to the variant annotation.
        :rtype: :class:`.VariantDataset`
        """
